...
const CreatePropertyForm: React.FC<CreatePropertyFormProps> = ({ 
    ...
    const schemaOptions: IDropdownOption[] = [
        { key: 'primitives', text: t('modelCreate.primitives'), itemType: DropdownMenuItemType.Header },
        ...
        { key: 'complexSchema', text: t('modelCreate.complexSchema'), itemType: DropdownMenuItemType.Header },
        { key: 'enum', text: 'Enum' },
    ...
    const parseSchema = (schema: DTDLSchema) => {
        const findDropdownOption = (key: string) => {
            const filtered = schemaOptions.filter(o => o.key === key.toLowerCase());
            return filtered[0];
        }
    ...
    const [schemaDropdown, setSchemaDropdown] = useState<IDropdownOption>(schemaInfo.option);
