...
const SceneView: React.FC<ISceneViewProp> = ({
        ...
        if (sceneRef.current?.meshes?.length) {
            ...
            hovMaterial.current =
                materialCacheRef.current[
                    currentRenderMode.id + currentRenderMode.meshHoverColor
                ] ||
                (materialCacheRef.current[
                    currentRenderMode.id + currentRenderMode.meshHoverColor
                    ...
                    currentRenderMode.lightingStyle
                ));
                ...
                for (const mesh of sceneRef.current.meshes) {
                    ...
                    if (!ignore) {
                        ...
                        mesh.useVertexColors =
                            currentRenderMode.lightingStyle < 1;
                        if (material) {
    ...
    const colorMesh = (mesh: AbstractMesh, color: string) => {
        ...
        const fresnelCol = currentRenderMode.fresnelColor || color;

        const materialId = currentRenderMode.id + col;
